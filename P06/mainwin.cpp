...
Mainwin::Mainwin(Store& store) : _store{&store} {
    ...
    //         P L A C E   O R D E R
    // Append Place to the Orders menu
    menuitem_place_order = Gtk::manage(new Gtk::MenuItem("_Place", true));
    menuitem_place_order->signal_activate().connect([this] {this->place_order();});  //ADD OUR OWN FUNCTION HERE
    ordersmenu->append(*menuitem_place_order);

    //         L I S T   O R D E R
    // Append Place to the Orders menu
    menuitem_list_orders = Gtk::manage(new Gtk::MenuItem("_List", true));
    menuitem_list_orders->signal_activate().connect([this] {this->list_orders();});  //ADD OUR OWN FUNCTION HERE
...
void Mainwin::list_orders() {
    ...
    for(int i=0; i<_store->num_sweets(); ++i) 
        s += /*"Quantity" + order._quantities(i) +*/ _store->sweet(i).name() + "  $" + std::to_string(_store->sweet(i).price()) + "\n";
    s += "</span>";
    data->set_markup(s);
#ifdef __STATUSBAR
    msg->set_text("");
...
void Mainwin::place_order() {
	...
	EntryDialog e3dialog{*this, "How many of this sweet do you want?"};
	e3dialog.set_text("Quantity");
	e3dialog.run();
	int quantity = std::stoi(e3dialog.get_text(), &sz);
	Sweet sweet{sweetname, sweetprice};
	_store->add(sweet);
	Order* _order = new Order;
	_order->add(quantity, sweet);
};
