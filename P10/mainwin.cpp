...
Mainwin::Mainwin() : shelter{new Shelter{"Mavs Animal Shelter"}} {
    ...
    // Append List to the Animal menu
    Gtk::MenuItem *menuitem_listadoptedanimal = Gtk::manage(new Gtk::MenuItem("_List Adopted", true));
    menuitem_listadoptedanimal->signal_activate().connect([this] {this->on_list_adopted_animal_click();});
    animalmenu->append(*menuitem_listadoptedanimal);
    ...
    // Append List to the Clients menu
    ...
    //           L I S T   A D O P T E D
    // Append List to the Clients menu
    Gtk::MenuItem *menuitem_listadoptedclient = Gtk::manage(new Gtk::MenuItem("_List Adopted", true));
    menuitem_listadoptedclient->signal_activate().connect([this] {this->on_list_adopted_client_click();});
    clientmenu->append(*menuitem_listadoptedclient);
...
void Mainwin::on_list_adopted_animal_click(){
    std::ostringstream oss;
    for(int i=0; i<shelter->num_animals(); ++i)
        oss << shelter->animal(i) << '\n'; 
    data->set_text(oss.str());
...
void Mainwin::on_list_adopted_client_click(){
    std::ostringstream oss;  
    for(int i=0; i<shelter->num_clients(); ++i) 
        oss << shelter->client(i) << '\n'; 
    data->set_text(oss.str());
