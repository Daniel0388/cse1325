...
void Mainwin::on_save_click() {
    shelter->set_filename("shelter.txt");
    std::ofstream ofs {shelter->get_filename()};
    if (!ofs) throw std::runtime_error{"can’t open save file"};
    ofs << shelter->name() << "\n"; //save shelter name
    for (int i = 0; i < shelter->num_animals(); i++) {
        //for each animal in the shelter, do something.
        Animal& animal = shelter->animal(i);
        ofs << animal.name() << "\n";
        ofs << animal.breed() << "\n";
        ofs << animal.family() << "\n";
        ofs << animal.gender() << "\n";
...
void Mainwin::on_open_click() {
    shelter->set_filename("shelter.txt");
    std::ifstream ist {shelter->get_filename()};
    if (!ist) throw std::runtime_error{"can’t open load file"};
    std::string s;
    std::getline(ist, s);
    shelter->set_name(s); //loads the shelter name
