...
class Prime_numbers {
  public:
    ...
    void find_primes(int lower, int upper) {
        std::thread t1(Prime_numbers::find_primes_thread, lower, upper);
        std::thread t2(Prime_numbers::find_primes_thread, lower, upper);
        std::thread t3(Prime_numbers::find_primes_thread, lower, upper);
        std::thread t4(Prime_numbers::find_primes_thread, lower, upper);
        t1.join();
        t2.join();
        t3.join();
        t4.join();
